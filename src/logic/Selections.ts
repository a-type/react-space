...
export class Selections extends EventSubscriber<{
	...
	set = (objectIds: Set<string> | Array<string>) => {
		const selectedIds =
			Array.isArray(objectIds) ? new Set(objectIds) : objectIds;
	...
	has = (objectId: string) => this.selectedIds.has(objectId);
	hasPending = (objectId: string) => this.pendingIds.has(objectId);
}
